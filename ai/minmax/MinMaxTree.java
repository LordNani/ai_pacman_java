...
public abstract class MinMaxTree {
    ...
    public MinMaxTree(MapGraph mapGraph, int depth, MapTile location, MapTile enemy_location, Board board) {
        this.board = board;
        allies_amount = 1;
        enemies_amount = 1;
...
//		root.getChildren().add(enemy_root);
        ...
        for (int i = 0; i < depth; ++i) {
            ...
            for (int j = 0; j < ally_moves.length; ++j) {
                ...
                for (MinMaxVertex move : last_level) {
                    if (!move.isEvaluated())
                        new_level.addAll(addChildrenByAncestor(move, ((j + 1) != ally_moves.length), generation_gap));
                }
            ...
            for (int j = 0; j < enemy_moves.length; ++j) {
                ...
                for (MinMaxVertex move : last_level) {
                    if (!move.isEvaluated())
                        new_level.addAll(addChildrenByAncestor(move, ((j + 1) == enemy_moves.length), generation_gap));
                }
                last_level = new_level;
            }
        }
        ...
        for (MapTile neighbour_of_father : enemy_move.getFather().getLocation().neighbours) {
            if (neighbour_of_father != null && !neighbour_of_father.equals(location_of_previous)) {
                ...
                if (neighbour_of_father.equals(enemy_move.getLocation()))
                    new_vertex.setValue(getCollisionValue(enemy_move));
            }
        }
    ...
    protected double evaluate(MinMaxVertex vertex) {
        if (vertex == null) {
        ...
        if (vertex.getChildren() == null || vertex.getChildren().isEmpty())
            return evaluateSituation(vertex.getFather(), vertex);
...
//            }
    ...
    protected MinMaxVertex getMinVertex(ArrayList<MinMaxVertex> children) {
    ...
    protected MinMaxVertex getMaxVertex(ArrayList<MinMaxVertex> children) {
        MinMaxVertex best_vertex = null;
